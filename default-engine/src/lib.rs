...
//!
//! [`AsyncEngineConnector`] drives connector-owned workflows one request at a time through native
//! asynchronous I/O without using [`Engine`] compatibility or a [`TaskExecutor`].
//! [`DefaultEngine`] implements Kernel's synchronous [`Engine`] compatibility interface, driving
//! its asynchronous I/O through a [`TaskExecutor`]. See the [executor] module for runtime
//! configuration.
