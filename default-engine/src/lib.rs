...
//!
//! [`AsyncEngineConnector`] drives connector-owned workflows directly through native asynchronous
//! I/O without an [`Engine`] or [`TaskExecutor`]. [`DefaultEngine`] implements Kernel's synchronous
//! [`Engine`] compatibility interface, driving its asynchronous I/O through a [`TaskExecutor`]. See
//! the [executor] module for runtime configuration.
