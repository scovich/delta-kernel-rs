...
//!
//! [`AsyncEngineConnector`] advances kernel tasks and serves their requests through the default
//! engine's object store, Arrow evaluator, and readers.
//!
//! Use [`AsyncEngineConnector::run`] when this connector should serve every kernel request. To
//! intercept requests such as catalog commit and publish operations, start the workflow yourself,
...
use delta_kernel::coroutine::{
    Cursor, GeneratorStep, Page, PageRequest, PagedOperation, Request, Workflow, WorkflowStep,
};
...
impl AsyncEngineConnector {
    ...
    /// Start `workflow` and serve every request until it completes.
    pub async fn run<W: Workflow>(&self, workflow: W) -> DeltaResult<W::Output> {
        let mut task = workflow.start();
        loop {
            match task.advance()? {
                WorkflowStep::Done(output) => return Ok(output),
                WorkflowStep::Request(request) => self.reply(request).await?,
            }
        }
    }
    ...
    pub async fn reply(&self, request: Request) -> DeltaResult<()> {
        match request {
            ...
            Request::CreateExpressionEvaluator(operation, reply) => {
                let result = ArrowEvaluationHandler
            ...
            Request::EvaluateExpression(operation, reply) => {
                let result = expression_evaluator(&operation.evaluator)
                    .and_then(|evaluator| evaluator.evaluate(operation.input.as_ref()));
                reply.send(result)
            }
            Request::EvaluateFilteredExpression(operation, reply) => {
                let result = expression_evaluator(&operation.evaluator).and_then(|evaluator| {
    ...
    async fn write_json_file(&self, operation: WriteJsonFile) -> DeltaResult<FileMeta> {
        ...
        loop {
            ...
            match input.advance()? {
                ...
                GeneratorStep::Yield(data, reply) => {
                    buffer.extend(to_json_bytes(std::iter::once(Ok(data)))?);
                    reply.send(Ok(()))?;
                }
                GeneratorStep::Request(request) => self.reply(request).await?,
            }
        }
...
async fn respond_paged<C, Op>(connector: &C, request: PageRequest<Op>) -> DeltaResult<()>
where
