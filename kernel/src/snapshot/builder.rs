...
use crate::coroutine::StaticWorkflow;
use crate::crc::Crc;
...
impl<Mode> SnapshotBuilder<Mode> {
    ...
    /// Build a [`Snapshot`] by driving [`workflow`](Self::workflow) through `engine`.
    ///
    ...
    /// Return a lazy workflow that produces a [`SnapshotRef`].
    ///
    ...
    #[internal_api]
    pub(crate) fn workflow(self) -> StaticWorkflow<SnapshotRef>
    where
        Mode: 'static,
    {
        static_workflow!(async move |channel| self.build_with_channel(channel).await)
    }

    async fn build_with_channel(self, channel: &Channel) -> DeltaResult<SnapshotRef> {
