...
impl<S> Transaction<S> {
        ...
        S: 'static,
    {
        ...
        let generator = GeneratorImpl::new(async move |yielder| {
            ...
            yielder.yield_all(add_actions).await?;
            Ok(())
        });
        Ok(generator)
    }
    ...
    fn generate_adds_for_dv_update(
        ...
        let generator = GeneratorImpl::new(async move |yielder| {
            let with_new_dv_evaluator = yielder
                ...
                .await?;
            let restored_add_evaluator = yielder
                .create_expression_evaluator(
                ...
                .await?;
            let with_data_change_evaluator = yielder
                .create_expression_evaluator(
