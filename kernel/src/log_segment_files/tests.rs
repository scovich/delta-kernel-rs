...
use super::*;
use crate::coroutine::engine::{run_workflow_with_storage, EngineConnector};
...
async fn test_list_ignoring_checkpoints_keeps_commits_and_crc() {
    ...
    let result = run_workflow_with_storage!(storage.as_ref(), None, async move |channel| {
...
#[test]
fn bound_listing_preserves_its_remaining_state() {
    let (log_root, _pulled, storage) = finite_listing_handler(5);
    let (first, remaining) = run_workflow_with_storage!(&storage, None, async move |channel| {
        let mut listing = list_delta_log_from_storage(channel, &log_root, 0, Version::MAX)?;
        let first = listing
            .next()
            .await?
            .ok_or_else(|| Error::internal_error("listing produced no files"))?;
        let mut remaining = Vec::new();
        while let Some(file) = listing.next().await? {
            remaining.push(file.version);
        }
        Ok((first, remaining))
    })
    .unwrap();
...
fn precancelled_token_stops_listing_before_any_storage_call() {
    ...
    let result = run_workflow_with_storage!(&storage, Some(token), async move |channel| {
...
fn mid_listing_cancellation_yields_terminal_error_not_silent_truncation() {
    ...
    .iterate_generator(StaticGenerator::new(move |channel| {
        list_delta_log_from_storage(channel, log_root, 0, Version::MAX).unwrap()
    }));
