...
fn bound_listing_preserves_its_remaining_state() {
    ...
    let (first, remaining) = drive_storage(&storage, None, async move |channel| {
        let listing = list_delta_log_from_storage(&log_root, 0, Version::MAX)?;
        let mut listing = listing.bind(channel);
        let first = listing
            .next()
            .await?
...
fn mid_listing_cancellation_yields_terminal_error_not_silent_truncation() {
    ...
    .iterate_generator(list_delta_log_from_storage(&log_root, 0, Version::MAX).unwrap())
    .unwrap();
...
fn backward_scan_checks_cancellation_between_windows() {
    ...
    let result = drive_storage(&storage, Some(token_ref), async move |channel| {
        LogSegmentFiles::list_with_backward_checkpoint_scan(channel, &log_root, vec![], 5_000).await
    });
    assert!(matches!(result, Err(Error::Cancelled)));
