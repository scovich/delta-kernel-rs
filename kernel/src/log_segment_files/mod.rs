...
use crate::coroutine::generator::generator;
use crate::coroutine::kernel::{Channel, Generator, GeneratorChannel};
use crate::coroutine::listing::{
    log_listing_bounds, BackwardListing, ForwardListing, ListingBounds,
};
...
/// Bind a low-level forward listing to `channel`.
///
/// The generator filters unrecognized and irrelevant entries and stops after `end_version`, but
/// does not merge a catalog-provided log tail, select or validate checkpoints, or construct
/// [`LogSegmentFiles`].
pub(crate) fn list_delta_log_from_storage<'task>(
    channel: impl Borrow<Channel> + Send + 'task,
    log_root: impl Borrow<Url> + Send + 'task,
    start_version: Version,
    end_version: Version,
) -> DeltaResult<Generator<'task, ParsedLogPath>> {
    let bounds = log_listing_bounds(log_root.borrow(), start_version, end_version)?;
    Ok(generator!(channel, async move |channel| {
        list_delta_log(channel, log_root.borrow(), bounds, end_version).await
    }))
}

async fn list_delta_log(
    channel: GeneratorChannel<'_, ParsedLogPath>,
    log_root: &Url,
    bounds: ListingBounds,
    end_version: Version,
) -> DeltaResult<()> {
    let mut page = channel.start_paged(ForwardListing::new(bounds)).await?;
    loop {
...
impl LogSegmentFiles {
    ...
    pub(crate) async fn list_commits(
        ...
        let mut fs_iter = list_delta_log_from_storage(channel, log_root, start, end)?;
        ...
        while let Some(file) = fs_iter.next().await? {
    ...
    pub(crate) async fn list_with_backward_checkpoint_scan(
        ...
        let mut page = channel.start_paged(BackwardListing::new(bounds)).await?;
        loop {
            let files =
