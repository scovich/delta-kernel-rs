...
impl SetTransactionScanner {
    ...
    pub(crate) fn get_all(
        ...
        run_workflow_with_engine!(engine, async move |channel| {
            Self::get_all_with_channel(log_segment, channel).await
        })
    }

    /// Scan the Delta Log for the latest `txn` action of every application id.
    #[allow(unused)]
    pub(crate) async fn get_all_with_channel(
        log_segment: &LogSegment,
        channel: &Channel,
    ) -> DeltaResult<SetTransactionMap> {
...
async fn scan_application_transactions(
    ...
    let replay = replay_for_app_ids_with_channel(log_segment, channel).await?;
    let mut replay = replay.bind(channel);
...
async fn replay_for_app_ids_with_channel(
    log_segment: &LogSegment,
    channel: &Channel,
...
fn replay_for_app_ids(
    ...
    let actions = run_workflow_with_connector!(connector, async move |channel| {
        replay_for_app_ids_with_channel(log_segment, channel).await
    })?;
...
mod tests {
    ...
    fn get_latest_transactions(
        ...
        let txn = run_workflow_with_engine!(&engine, async move |channel| {
            SetTransactionScanner::get_one(log_segment, app_id, channel).await
        });
        (
