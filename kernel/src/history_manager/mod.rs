...
async fn get_earliest_published_commit_version(
    ...
    let mut listing = list_delta_log_from_storage(log_root, 0, Version::MAX)?.bind(channel);
...
async fn get_earliest_recreatable_commit(
    ...
    let mut listing = list_delta_log_from_storage(log_root, 0, Version::MAX)?.bind(channel);
    while let Some(parsed_log_path) = listing.next().await? {
...
/// Return a lazy workflow that locates the earliest table version at `log_root`.
///
...
#[tracing::instrument(name = "get_earliest_commit", skip_all)]
pub fn earliest_commit_workflow(
    log_root: Url,
    earliest_ratified_commit_version: Option<Version>,
    commit_type: HistoryCommitType,
) -> impl Workflow<Output = Version> {
    WorkflowImpl::new(async move |channel| match commit_type {
        HistoryCommitType::Published => {
...
/// Return the earliest table version by driving [`earliest_commit_workflow`] through `engine`.
///
/// # Parameters
/// - `engine`: kernel engine used to list `log_root`.
/// - Other parameters have the same meaning as [`earliest_commit_workflow`].
///
...
pub fn get_earliest_commit(
    ...
    let workflow = earliest_commit_workflow(
        log_root.clone(),
        earliest_ratified_commit_version,
        commit_type,
    );
