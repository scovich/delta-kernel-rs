...
pub(crate) async fn try_read_crc_file_with_channel(
    channel: &Channel,
...
/// it. The failure is logged and metered by [`try_read_crc_file_with_channel`]'s instrumentation.
pub(crate) async fn read_crc_file_or_none(
    channel: &Channel,
    crc_file: &ParsedLogPath,
) -> Option<Arc<Crc>> {
    try_read_crc_file_with_channel(channel, crc_file)
        .await
        .ok()
        .map(Arc::new)
}

/// Test helper that drives [`try_read_crc_file_with_channel`] through a legacy [`Engine`].
#[cfg(test)]
pub(crate) fn try_read_crc_file(engine: &dyn Engine, crc_path: &ParsedLogPath) -> DeltaResult<Crc> {
    ...
    run_workflow_with_engine!(engine, async move |channel| {
        try_read_crc_file_with_channel(channel, &crc_path).await
    })
}
