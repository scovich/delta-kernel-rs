...
and `Generator` values whose tasks expose typed requests to the connector. Synchronous
...
**Coroutine execution:** connectors call `start` only for root operations they will drive.
Kernel code with an existing `Channel` composes workflows through `run_with` and generators through
`bind`; starting there creates an unnecessary independent task. Drivers complete each request's
`Reply` before advancing the same task. `AsyncEngineConnector` is the default native async driver.
