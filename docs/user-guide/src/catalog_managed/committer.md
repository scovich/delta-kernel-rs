...
New connector-controlled integrations normally serve `Request::Commit` directly; start with
[Connector-driven commits](#connector-driven-commits). The `Committer` trait below documents the
synchronous Engine compatibility path.
...
A Kernel transaction workflow emits `Request::Commit(commit, reply)`. Your connector serves
`commit` through its chosen committer and calls `reply.send(result)`.
